...
app.get("/notify/:id", async (req, res) => {
  try {
    ...
    const notify = await Voucher.findById(NotifyId);
    res.json(notify);
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Failed to get Notify", error });
  }
});
// PUT
app.put("/notify/:id", async (req, res) => {
  try {
    const NotifyId = req.params.id;
    const { order_id, user_id, isActive } = req.body;

    const notify = await Notify.findByIdAndUpdate(
      NotifyId,
      { order_id, user_id, isActive },
      { new: true }
    );
    ...
    console.log(error);
    res.status(500).json({ message: "Failed to update Bill status", error });
  }
});
//POST
...
app.get("/bill/:id", async (req, res) => {
  try {
    ...
    const bill = await Voucher.findById(BillId);
    res.json(bill);
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Failed to get Bill", error });
...
app.post("/bill", async (req, res) => {
  try {
    const { order_id, discount, surcharge, total, date } = req.body;
    // Kiểm tra dữ liệu đầu vào
    if (!order_id || !total || !date) {
...
app.put("/bill/:id", async (req, res) => {
  try {
    ...
    const bill = await Bill.findByIdAndUpdate(
      BillId,
      { order_id, discount, surcharge, total, date },
      { new: true }
    );
...
app.post("/vouchers", async (req, res) => {
  try {
    ...
    const { codeVoucher, discount, point, status } = req.body;
...
app.put("/vouchers/:id", async (req, res) => {
  const { id } = req.params;
  const { discount, point, status, codeVoucher } = req.body;
  console.log(discount, point, status);
  ...
  try {
    const updatedVoucher = await Voucher.findOneAndUpdate(
      { _id: id },
      { discount, point, status, codeVoucher },
      { new: true }
    );
...
app.post("/orderchef", async (req, res) => {
  try {
    const { table_id, products, date, status, note } = req.body;
    // Kiểm tra dữ liệu đầu vào
...
app.put("/order/:id", async (req, res) => {
  const { id } = req.params;
  const { table_id, sub_total, products, user_id, date, status } = req.body;
  ...
  try {
    ...
    const updatedOrder = await Order.findByIdAndUpdate(
      id,
      { table_id, sub_total, products, user_id, date, status },
...
// DELETE ID VOUCHER
app.delete("/users/:userId/voucher/:voucherId", async (req, res) => {
  ...
  try {
    if (mongoose.connection.readyState === 1) {
      ...
      // Remove the voucher from the user's vouchers
      ...
      res.json({ message: "Voucher removed successfully", user });
    } else {
      throw new Error("MongoDB not connected");
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Failed to remove voucher", error });
  }
});
// Tạo endpoint PUT để cập nhật user theo id
app.put("/users/:id", async (req, res) => {
  const { id } = req.params;
  const { username, phone, isAdmin, email, password, point, vouchers } =
    req.body;
  try {
    if (mongoose.connection.readyState === 1) {
      const updatedUser = await User.findByIdAndUpdate(
        id,
        { username, phone, isAdmin, email, password, point, vouchers },
        { new: true } // Trả về document đã được cập nhật
      );
      if (updatedUser) {
